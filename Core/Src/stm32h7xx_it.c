...
void DMA1_Stream2_IRQHandler(void)
{
  ...
  if(DMA1->LISR & DMA_LISR_TCIF2)
  {
    ...
    //TIM2->CR1 &= ~TIM_CR1_CEN; // притупим таймер основной
       // StopAllTIM(2);
    // TST_KTB(0); // конец "МОДУЛЬ" 1 ( останов основного цикла накопления)
...
// прерывание по окончании передачи по оптике UART5
  ...
  if(DMA1->HISR & 0x800)
  {
    HAL_UART_DMAStop(&huart5);
     TxOptBusy = 0; // сбросим признак занятости оптики
      NeedFreq = 1;
  }
...
void TIM1_CC_IRQHandler(void)
{
    ...
    // здесь перестроим Таймеры на новые положения если надо
  {
    ...
    StopAllTIM(0); // останов таймеров без Т1, для подготовки устанвок следующего измерения
...
void EXTI15_10_IRQHandler(void)
{
  ...
  if (__HAL_GPIO_EXTI_GET_IT(BUT_START_Pin) != 0x00U)
  {
    ...
    rawPressKeyS = 1;
    TimePressKeyS = HAL_GetTick();
    if((TimePressKeyS - TimeEndKeyS)<500)
      rawPressKeyS = 0;
