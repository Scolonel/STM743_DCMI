...
uint32_t PointForDist = 5000; // число точек съема (зависит от уст диапазона (дистанция)
...
//    }
    ...
    if(ModeUSB)
    {
      ...
      {
    case 1:
    sprintf(Str,"pic 460,0,11яяя");//black
      break;
    case 2: // отключили USB
    sprintf(Str,"pic 460,0,12яяя");//empty
      break;
    case 3:
    sprintf(Str,"pic 460,0,9яяя");//RED
      break;
      }
      ModeUSB = 0;
