...
uint8_t DistBad=0; // дистанция плохая линия длинней , для Averaging, 
...
uint32_t g_CountDMA = 0; // число совершенных циклов DMA с учетеом прореживания и всех накоплений
...
uint32_t g_Tim5Set; // текущее значение счетчика для установки времени начала суммирования
...
uint8_t g_ErrTO = 0; // признак выхода по таймауту при общении с SD картой
...
DWORD CountTimerPA = 0; // счетчик времен для режима автомат PM 
// Группа основных перемнных участвующих в накоплении
float fRawData[RAWSIZE]; // блок накопления в основной памяти
...
uint8_t g_STindx_IM = 0; //режим СуперТеста, индекс длительности импульса 
...
uint8_t RunInRealTime = 0; // измеряем в режиме RealTime
...
//    HAL_Delay(5);
  ...
  //  NEX_Transmit((void*)Str);    //
  //    sprintf((void*)Str,"t0.bco=11111яяя");
  //    NEX_Transmit((void*)Str);//
  //       StartRecievNEX (500);
  ...
  //  sprintf((void*)Str, "page 0яяя"); // < START>
  //  NEX_Transmit((void*)Str);    //
  //    sprintf((void*)Str,"t1.bco=22222яяя");
  //    NEX_Transmit((void*)Str);//
  //  HAL_Delay(100);
  ...
  //    sprintf((void*)Str,"t2.bco=33333яяя");
  //    NEX_Transmit((void*)Str);//
  //LED_KTS(0);
  HAL_Delay(50);
  StartRecievNEX (400);
  ...
  //    sprintf((void*)Str,"t3.bco=44444яяя");
  //    NEX_Transmit((void*)Str);//
  //LED_KTS(0);
  ...
  //  if(!KnowLCD)
  //  {
  //    sprintf((void*)Str,"t8.bco=54321яяя");
  //    NEX_Transmit((void*)Str);//
  //  }
  ...
  HAL_DAC_SetValue(&hdac1,DAC_CHANNEL_2,DAC_ALIGN_12B_R,182);
...
// 1 бит - 1 место, 2бит - второе место, 3 бит - 3 место, комбинации возмжны из заявленных длин волн в значащих местах то есть не 0
  ...
  { 
    ...
    // вынесем сюда управление источником в режиме CW* и привяжем к таймеру
    // счетчику  1мС
    ...
    {
      Run_SCWP(); // вызовем функцию управления лазером по признакам уставливаемым
...
//      }
      ...
      if(++PeriodIntADC > 15)// 450 mS
      {
        ...
        PeriodIntADC = 0; // это каждые ~450 мС
        Tik_045 = 1; 
        CountSeqMeas++;
        // проверим ошибку и просигнализируем
        if(g_ErrTO)
        {
          sprintf((void*)Str,"xstr 10,150,460,40,2,RED,WHITE,1,1,1,\"%d\"яяя",g_ErrTO); //  сообщение об ошибке FW LCD
          NEX_Transmit((void*)Str);//
          g_ErrTO = 0;
        }
      }
    ...
    // проверка приема по UART EXT - перенесено ниже после цикла режима
    // вдруг там "убиваем" принятую команду
      ...
      if(ModeUSB) // устанавливаем когда пишем или читаем по USB SDCard
      {
        ...
        if(Tik_045) //  каждые .45 сек
        {
          ...
          case 3:
            ...
            sprintf((void*)Str,"pic 144,94,%dяяя",(CountSeqMeas%3)+16);
            //sprintf((void*)Str,"pic 144,94,53яяя");
            NEX_Transmit((void*)Str);//
            MemMsgModeUSB = 1; // признак работы USB для индикации доп строчки
            ModeUSB=2; // сбросится (инкрмент) в прерывании 1 мС
          ...
          default:
            ...
            // скорей всего ModeUSB уже и так  =0
            ModeUSB = 0;
            CmdInitPage(NumCurrPage);
            // но он же и может стереть разрешенную флэшку
            break;
          }
          Tik_045 =0;
        }
...
//}
    ...
    void StartRunFirst(unsigned AddIndx) //
    {
      ...
      // 4 км - время суммирования ~ 80uS
      // 8 км - время суммирования ~ 160uS
      // 16 км - время суммирования ~ 320uS
      TIM5->ARR = 15;
      TIM5->CCR4 = 5;
      // восстановим частоту - так на всякий случай
       ...
       {
       case 0: //0.5 2.0 km - 33.3nS*700=24uS = Тнак (прореживание = 8)
         ...
         g_Tim5Set = 7;
         TIM5->CCR4 = g_Tim5Set; //через 1.666 мкС + 42 мкС(сумм 700) меньше 105 мкС
       ...
       case 1: //4 km - 33.3nS*1400=47uS = Тнак (прореживание = 4)
         ...
         //TIM4->ARR = TIM4->CCR4 + (8+1)*2;
         g_Tim5Set = 11;
         TIM5->CCR4 = g_Tim5Set; //через 1.666 мкС + 84 мкС(сумм 1400) меньше 100 мкС
         TIM5->ARR = g_Tim5Set + 1;
         break;
       case 2: //8 km - 33.3nS*2800=94uS = Тнак (прореживание = 2)
         ...
         //TIM4->ARR = TIM4->CCR4 + (8+1)*2;
         g_Tim5Set = 239;
         TIM5->CCR4 = g_Tim5Set; // +168 мкС (сумм 2800) меньше 200мкС
         TIM5->ARR = g_Tim5Set + 1;
         break;
       ...
       case 4: //32km - 330uS 66.6nS*5600=373uS = Тнак
         ...
         g_Tim5Set = 90000; // 375
         TIM5->CCR4 = g_Tim5Set;//50*240; 373uS = Тнак < через 50 мкС + 336 мкС (сумм 5600) < 420 мкС
       ...
       case 5: //64km - 660uS 133.3nS*5600=750uS = Тнак
         ...
         TIM5->CCR4 = g_Tim5Set;//240*420; 750uS < через 420 мкС + 336 мкС (сумм 5600) < 800 мкС
       ...
       case 6: //128km - 1300uS 266.6нС *5600=1493uS = Тнак
         ...
         TIM5->CCR4 = g_Tim5Set;//288000;//240*1200;1493 мкС < через 1200 мкС + 336 мкС (сумм 5600) < 1493 мкС
...
void Error_Handler(void)
{
  ...
  if(g_CodeErrorSoft)
  {
    ...
    {
    case 1:
    sprintf((void*)Str, "t0.txt=\"! ОШИБКА !\"яяя"); // Нет карточки SD
    NEX_Transmit((void*)Str);    // 
    HAL_Delay(50);
    sprintf((void*)Str, "t1.txt=\"Установите карточку SD\"яяя"); // Нет карточки SD
    NEX_Transmit((void*)Str);    // 
    HAL_Delay(5);
