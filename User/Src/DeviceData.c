...
// 0.5-2 км,  4км,         8км,         16км,        32км,        64км,        128км
// 48,        24,          12,          6,           3,           2,           1,
// 144МГц,    72МГц,       36МГц,       18МГц,       9МГц,        6МГц,        3МГц
// 
// на STM
// 8,         4,           2,           1,           1,           1,           1,
// 240МГц,    120МГц,      60МГц,       30МГц,       15МГц,       7.5МГц,      3.75МГц
// погонное затухание на один отсчет, в зависимости от диапазона и длинны волны
// расчитанные для коэфф преломления 1.4681, 
// погонное затух на км, 850-1.81, 1310-0.33, 1550-0.19, 1490:1625-0.21
...
// таблица для STM32
// погонное затух на км, 850-2.1, 1310-0.33, 1550-0.19, 1490:1625-0.21
const float LSACoef[WAVE_LENGTHS_NUM][LENGTH_LINE_NUM] = {
  {0.893416661,	1.786833322,	3.573666644,	7.147333288,	14.29466658,	28.58933315,	57.1786663}, //850 ММ
  {0.140394047,	0.280788093,	0.561576187,	1.123152374,	2.246304748,	4.492609495,	8.985218991},//1310
  {0.089341666,	0.178683332,	0.357366664,	0.714733329,	1.429466658,	2.858933315,	5.71786663}, //1490
  {0.080832936,	0.161665872,	0.323331744,	0.646663488,	1.293326976,	2.586653952,	5.173307904},//1550
  {0.089341666,	0.178683332,	0.357366664,	0.714733329,	1.429466658,	2.858933315,	5.71786663}  //1625
};
...
uint8_t ModeSCWP=0; // режим времени для переключения лазеров
uint16_t TimeSCWP=0; // счетчик времени для переключения лазеров
...
void SetIndexIM (BYTE Index) // установка индекса длины Pulse
{
  ...
  case 0: //2km
  case 1: //4km
    //if (Index>WIDTH_PULSE_NUM-4)Index = WIDTH_PULSE_NUM-4; //делаем ограничение на 1000нС (3) без 20000 при 11 вариантах
    if (Index>WIDTH_PULSE_NUM-4)Index = WIDTH_PULSE_NUM-4; //делаем ограничение на 1000нС (3) без 20000 при 10 вариантах A.K.
    break;
  case 2: //8km
  case 3: //16km
  case 4: //32km
  case 5: //64km
    ...
    //if (Index>WIDTH_PULSE_NUM-2)Index = WIDTH_PULSE_NUM-2; // делаем ограничение на 3000нС (2) без 20000 A.K.
...
//  if (Index <6)
//  if (Index <8)
  if (Index <7)  //A.K. 21.05.2026 выкл фильтра до 1000 нС
  {
    SW_FLTR(OFF);
...
float TauImp (void)
{
  ...
  float Data;
    ...
    Data = (ADCPeriod*MultIndex[GetIndexLN()])/NumPointsPeriod[GetIndexLN()];
...
void Run_SCWP(void) // каждые 30 мС
{
  // ModeSCWP - признаки работы  источника в режиме CW*
  // 0 - молчим
  // 1 - посылка сообщения
  // 2 - начало излучения постоянное
  ...
  if(ModeSCWP) // ~3.5 сек
  {
    ...
    case 1: // 90мС
      ...
      HAL_UART_Transmit_DMA(&huart5, (void*)StrTXOpt,strlen((void*)StrTXOpt)); // выдаем (время выдачи ~210мС)
      TxOptBusy = 1;
      ...
      //HAL_Delay(200);
      break;
    case 2: // через 500 мС после старта передачи... 
      PHLD(1); // on CW 
      break;
    case 3: // через 500 мС после старта передачи... 
    PHLD(0); // off CW
    // переключаем ЛАЗЕРЫ
...
{
  if(NeedFreq) // для режимов излучения источника 270Гц и 2кГц
  {
