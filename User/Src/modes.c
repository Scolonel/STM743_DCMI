...
#define RESISTOR_PWR 62. // номинал резистора (кОм) в делителе для контроля напряжения батарейки
// на макетках стоит 50кОм (2||100кОм)
...
const int KeyPoints[PNTSNUM] = { 105, 146, 229, 395, 726, 1389, 2714 }; // порги определения индекса установленной длины теперь у нас 5300 точек реальных, снимаем 5600
...
struct tag_PON PONI;
struct tag_PON R_PONI; // что читаем из памяти при записи в файл
...
BYTE SW_13=0; // признак переключения длины волны 1300/1310
...
void ModeFuncTmp(void)
{
  ...
  if(SystLogWord)
  {
    if(SystLogWord & KEYB_S) // увеличим яркость
      SetBrightness(100);
    if((SystLogWord & TM_15)&&(!RunInRealTime)) // реальное время // уменьшим яркость
      SetBrightness(40);
...
{ 
  ...
  {
    // восстанавливаем 1310 если в рефлектометре было 1300
    SW_13 = 0;
    // наименование прибора
    GetDeviceLabel( Str, NameDB.Ena_DB  );
...
void ModeSetupOTDR(void) // режим установок рефлектометра CHECK_OFF
{
  ...
  case 0: //длина волны
    ...
    if ((PRESS(BTN_OK))&&(getStateButtons(BTN_OK)==SHORT_PRESSED))
    {
      ...
      //      g_AutoSave = 0; 
      if((g_AutoSave)||(GetIndexVRM()>3))
      {
        // выключаем авто сохранение
        g_AutoSave = 0;
        // если нет признака SW_13, то устанавливаем его
        // иначе инвертируем если это длина волны 1310 и признак ФСТЭК
        if((ConfigDevice.ForFBI)&&(ConfigDevice.PlaceLS[SettingRefl.SW_LW]==1310)) // признак переключения длины волны 1300/1310
        {
          if(SW_13)
            SW_13 = 0;
          else
  ...
  if(g_FirstScr)
  {
    ...
    // столбцы названий 
    // переключатель вывода в зависимости от комбинации
    // востанавливаем имя влокна(префикс файла)
  ...
  if(g_NeedScr)
  {
    ...
    if (g_AutoSave)
    {
      ...
      sprintf(Str,"t9.txt=\"\"яяя");// пустая строка
    }
    else
    {
...
void ModeStartOTDR(void) // режим накопления рефлектометра
{
  ...
  case INPUTOTDR:
    ...
    if(g_NeedScr) //
    {
      sprintf(Str,"t1.txt=\"%s\"яяя",MsgMass[34][CurrLang]); // излучение на входе
      NEX_Transmit((void*)Str);// 
      sprintf(Str,"t6.txt=\"???\"яяя"); // 
      NEX_Transmit((void*)Str);// 
      g_NeedScr = 0;  
  ...
  case CALKORLBEG:
    ...
    sprintf(Str,"t6.txt=\"%s\"яяя",MsgMass[35][CurrLang]); // "НЕТ"
    NEX_Transmit((void*)Str);// 
  ...
  case QUALITYBEGIN:
    ...
    if(g_NeedScr)
    {
      ...
      //NEX_Transmit((void*)Str);// 
      sprintf(Str,"t2.txt=\"%s\"яяя",MsgMass[32][CurrLang]); // входной разъем
      NEX_Transmit((void*)Str);// 
      sprintf(Str,"t7.txt=\"%s\"яяя",MsgMass[33][CurrLang]); // тест
      NEX_Transmit((void*)Str);// 
  ...
  case SEARCHENDLINE:
    ...
    Noise = Noise/100;
    // порог принятия решения о конце линии
    // раньше было 2 - 1.5 дБ, что как бы мало,
    // так как при съеме даже короткой линии 1.7 км при импульсе в 1000 нс
    // затяжка после перегрузке в конце тянется аж до 5.8 км, при разнице в 1.5 дБ
    // Д.Б. предложил поиск выше в 10 раз (5дБ)  над пиковым уровнем шумов ,
    // проерим для 3 дБ (4раза)
    //RawData[0] = Noise + (LocalMax-Noise)*2;
    RawData[0] = Noise + (LocalMax-Noise)*10;
    ...
    */
    LengthOK = YES;
    CurrentSumDelay = 1;
    Mean = Scan( RawData, RAWSIZE , RawData[0] );// точка отражения , как бы конец линии в режиме 128 км
    //LengthLine = Mean;
    // шаг полученного значения  266.6 нС (27м)
    // при этом 64 точки до импульса
    // таблица порогов длин линий при этом измерении
    // 2 км - 
    //Mean = (DWORD)((0.2666*Mean)+1);// длина линии в мкС
    // контроль с установленной длинной
    ...
    if (IndxAddBad>ShadowIndexLN) 
    {
      ...
      // расчет некой задержки, которая сейчас не используется
    ...
    {
      // подбор диапазонов и соотв длительностей импульсов врежиме АВТО
  ...
  case LASTCHECK:
    ...
    if ((ShadowIndexIM > 6) && (GetIndexVRM()!=4)) //1000ns для первого измерения устанавливаем импульс если задан большой 3...10мкс
    {
      //SetIndexIM(5); //1000ns для первого измерения устанавливаем импульс если задан большой 3...10мкс
      // 21.04.2026
      //SetIndexIM(7); //1000ns для первого измерения устанавливаем импульс если задан большой 3...10мкс
      // 21.05.2026 A.K.
      SetIndexIM(6); //1000ns для первого измерения устанавливаем импульс если задан большой 3...10мкс
  ...
  case AVERAGING:
    ...
    {
      RunInRealTime = 1; // измеряем в режиме RealTime
      CntNumAvrg = 0; // обнуляем счетчик накоплений
      memset( RawData, 0, RAWSIZE * sizeof(DWORD) );
...
{
  // основные параметры для отображения рефлектограммы
  // из параметров измерения -  функция PointsInImpuls(0)
  // индекс длительности импульса, и индекс длины линии (сейчас из текущих,
  // который выбираем в установках рефлектометра перед запуском измерения...)
  //
  char Str[48];
  char Stra[24];
  ...
  if(g_NeedScr)
  {
    ...
    if(TypeLCD)
      sprintf( Str,"xstr 330,0,120,24,3,GREEN,BLACK,0,1,3,\"%s\"яяя",Stra); // 3_5(3-24) (№ шрифта-Размер) 0
    else
      sprintf( Str,"xstr 260,0,140,24,2,GREEN,BLACK,0,1,3,\"%s\"яяя",Stra); // 3_2(2-24) (№ шрифта-Размер) 0
    NEX_Transmit((void*)Str);// 
    //CreatDelay (20000);
...
void ModeSelectOLT(void) // режим выбора типа тестера CHECK_OFF
{
  ...
  if(g_NeedScr)
  {
    ...
    if(RedEye)
    {
      ...
      //CntrlRE (); // функция контроля красным глазом
    }
...
void ModeSourceOnly(void) // режим работы тестера только источник CHECK_OFF
{
  ...
  if (g_FirstScr)
  {
    ...
    if(RedEye)
    {
      ...
      NEX_Transmit((void*)Str);    // 
      // это была рамка для режима но не в том месте и для 3.2 дюйма индикатора
...
void ModeSaveOLT(void) // режим сохранения результатов измерителя CHECK_OFF
{
  ...
  if (g_FirstScr)
  {
    ...
    sprintf(Str, "t8.txt=\"№ %04d\"яяя", GetCellMem(0)+1);
    NEX_Transmit((void*)Str);    // Сброс счетчика
...
void ModeSelectMEM(void) // режим выбора работы с памятью CHECK_OFF
{
  ...
  if (g_FirstScr)
  {
    ...
    if (PowerMeter) // есть измеритель
    {
      ...
      sprintf(Str, "t5.txt=\"%3d\"яяя",MaxMemPM-GetCellMem(0));
      //NEX_Transmit((void*)Str);    // ИЗМЕРИТЕЛЬ (число ячеек)
      //sprintf(Str, "t5.txt=\"%d/%d\"яяя",GetCellMem(0),MaxMemPM);
      NEX_Transmit((void*)Str);    // ИЗМЕРИТЕЛЬ (число ячеек)
      
      //sprintf(Str, "t6.txt=\"%s\"яяя", MsgMass[44][CurrLang]);
      //NEX_Transmit((void*)Str);    // ОЧИСТКА
      sprintf(Str, "t6.txt=\"USB\"яяя");
      NEX_Transmit((void*)Str);    // ОЧИСТКА- пустая
    }
    else  // измерителя нет
    {
      //sprintf(Str, "t4.txt=\"%s\"яяя", MsgMass[44][CurrLang]);
      //NEX_Transmit((void*)Str);    // ОЧИСТКА
      sprintf(Str, "t4.txt=\"USB\"яяя");
      NEX_Transmit((void*)Str);    // ОЧИСТКА- пустая
      sprintf(Str, "t5.txt=\"\"яяя");
  ...
  if ((PRESS(BTN_OK))&&(getStateButtons(BTN_OK)==SHORT_PRESSED))
  {
    ...
    case 1: // переход в память рефлектограмм
      ...
      break;
    case 2: // переход в  память измерителя (выбор работы)
      if (PowerMeter) // есть измеритель
      {
        // go to wiev memPM
        if (GetCellMem(0))
        {
          ...
          // посылка команды переключения окна на Mem_OLT_Choose (выбор действий)  
          ClrKey (BTN_OK);
          CmdInitPage(21);
          HAL_Delay(100);
        }
      }
      else   // !!! убрали очистку памяти
      {
        myBeep(10);
        SetMode(ModeReadUSB);
...
void ModeReadUSB(void) // режим чтения по USB памяти флэшки установка признака
{
  ...
  if (g_FirstScr)
  {
    ...
    sprintf(Str, "t0.txt=\"%s\"яяя", MsgMass[132][CurrLang]); 
    NEX_Transmit((void*)Str);    // Переподключите
    
    sprintf(Str, "t1.txt=\"%s\"яяя", MsgMass[133][CurrLang]);
    NEX_Transmit((void*)Str);    // кабель USB
    
    sprintf(Str, "t2.txt=\"%s\"яяя", MsgMass[134][CurrLang]);
...
void ModeClearMEM(void) // режим освобождения памяти измерителя CHECK_OFF
{
  ...
  if ((PRESS(BTN_OK))&&(getStateButtons(BTN_OK)==SHORT_PRESSED))
  {
    ...
    case 1: // вызов окна просмотра
      ...
      //SetMode(ModeSelectMEM);
      break;
    case 2: // записываем в файл и остаемся здесь же
      //      if (PowerMeter)
      //      {
      //        DeletedAllCell (); // удаление всех записей измерителя
      myBeep(10);
      TimeSavePM = RTCGetTime(); // сохраняем время создания файла
      // функция записи файла измерений измерителя
      sprintf( Str,"xstr 10,150,460,40,2,RED,WHITE,1,1,1,\"%s\"яяя","СОХРАНЕНО"); //  сообщение об ошибке FW LCD
      NEX_Transmit((void*)Str);//
      SaveFilePM();
      ...
      //      }
      break;
    case 3: // чистим память для измерителя 
      myBeep(10);
      DeletedAllCell (); // удаление всех записей измерителя
      sprintf( Str,"xstr 10,150,460,40,2,RED,WHITE,1,1,1,\"%s\"яяя","ОЧИЩЕНО"); //  сообщение об ошибке FW LCD
      NEX_Transmit((void*)Str);//
      HAL_Delay(800);
      myBeep(30);
      //g_FirstScr = 1; // Need reDraw Screen
  ...
  if (g_FirstScr)
  {
    // здесь заполняем данными поля нового индикатора
    // не требущие изменения при первичной инициализации     
    sprintf(Str, "t0.txt=\"%s\"яяя", MsgMass[126][CurrLang]); 
    NEX_Transmit((void*)Str);    // Память тестера
    
    sprintf(Str, "t1.txt=\"%s\"яяя", MsgMass[127][CurrLang]);
    NEX_Transmit((void*)Str);    // Просмотр
    
    sprintf(Str, "t2.txt=\"%s\"яяя", MsgMass[128][CurrLang]);
    NEX_Transmit((void*)Str);    // Сохранить     
    
    sprintf(Str, "t3.txt=\"%s\"яяя", MsgMass[129][CurrLang]); 
    NEX_Transmit((void*)Str);    // Очистить удалить
    g_FirstScr = 0;
    g_NeedScr = 1;
  }
  if (g_NeedScr)
...
void ModeViewMemOLT(void) // режим просмотра памяти измерителя CHECK_OFF
{
  ...
  if (g_NeedScr)
  {
    ...
    NEX_Transmit((void*)StrO);// 
    switch (R_PONI.Rez) // далее рисуем в зависимости от того какая каритнка
    {
    case MANUAL:
...
void SetBrightness(uint8_t Bright)
{
  ...
  if(Bright<=100)
  {
  sprintf(str, "dim=%dяяя",Bright); // < событиe >
  NEX_Transmit((void*)str);    //
  }
  else
  {
  if(Bright<=200)
  {
  sprintf(str, "dims=%dяяя",Bright-100); // < событиe >
  NEX_Transmit((void*)str);    //
  }    
  }
  //HAL_Delay(30);
