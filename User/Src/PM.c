...
int AcquireShZeroLowRng(void)                          // Измерение уровней смещения на нижнем диапазоне (переустановка нуля)
{
    ...
    // установлен диапазон 0  ?
  {
    ...
    sprintf(Str,"t1.txt=\"%d\"яяя",AverageData); // 
    NEX_Transmit((void*)Str);    //
    ...
    StBTN = GetExpand (); // прочитаем кнопки
     if(!(StBTN & BTN_OK))
     {
       // можно записать в память!!!
       myBeep(300);
       WriteNeedStruct(0x02);
       sprintf(Str, "t1.bco=REDяяя"); // красный
       NEX_Transmit((void*)Str);//
