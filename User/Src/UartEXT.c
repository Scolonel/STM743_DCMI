...
void DecodeCommandRS (void)
{
  ...
  if (Reciev==END_UART)                //Ежели приём команды закончен - обработка
  {
    ...
    case ';':
      ...
      // запрос признака ФСТЭК 
      //  ;syst:FSB?
      if (!memcmp ((void*)RX_Buf, ";SYST:FSB?",10)) //
      {
        
        sprintf(BufString,"%d\r",GetCfgFSB ()); // получение признака ФСТЭК; // 
        UARTSendExt ((BYTE*)BufString, strlen (BufString));
        NeedTransmit = 1;
      ...
      if (GetCurrentModeDevice()==MODESETUP)
      {
        ...
        if(1)
        {
          ...
          if (!memcmp ((void*)RX_Buf, ";SET:SHAN ",10)) //
          {
            ...
            // ;set:shan  - установка значения делителя
