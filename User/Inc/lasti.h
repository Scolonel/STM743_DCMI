...
//  v 604b(105) 11.02.2026           (1768212000) 2026.01.12 10:00:00
    // оптимальная работа вновь собранного многомодового AR-рефлектометра
    // 7104AR - индикация динамического диапазона.
    // ВНИМАНИЕ! Очень чувствителен к установке смещения, при неправильной установке
    // смещения - отображения рефлектограмм не предсказуемы.
//  v 604c(106) 16.02.2026           (1768212000) 2026.01.12 10:00:00
    // блокировка MSC только в приеме и передаче, дает эффект без зависания CDC
    // НО! проскакивает инициализация не существующего (не отвечающего) диска в Виндовс
    // Диск как бы есть, но его нет!!! и иногда при старте просит его форматировать,
    // запуск форматирования проходит если запустить, но реакции нет.
//  v 604d(107) 20.02.2026           (1768212000) 2026.01.12 10:00:00
    //НЕОЖИДАННО! Выплыли две ошибки, при программировании индикатора, не было ожидания 
    //ответа при подаче команды, получаласть задержка и был "ложный" ответ при переключании в режим
    //программирования индикатора, в программу транслировался ответ от индикатора так как он уже
    // переключился. Программа не получив нужного ответа, снова запрашивала установку, но получала в 
    // в ответ предыдущей запрос, и при задании начала программирования снова получала commok, вместо 0x05.
    // также при работе с программой версии 270, при установке диапазона 16 км, 
    // происходила синхронизация обменов, и на конец измерения в буфер приема
    // команд прилетал запрос дампа, который еще не был обработан, НО программа
    // приняв "END" - запрашивала IDN, но в ответ получала дамп...
    // Особенность временных интервалов случайно синхронизировалось...
    // теперь после подачи "END" - я туплю 250 мС и обнуляю все взведенные команды, игнорируя их.
//  v 604e(108) 25.02.2026           (1768212000) 2026.01.12 10:00:00
    // Подбор временных интервалов определения длительности 3 секундного
    // накопления, для адекватной работы.
    // добавил расчет шумов на 16 км в конце линии
    // корректировка временных интервалов при измерениии от программы
//  v 604f(109) 02.03.2026           (1768212000) 2026.01.12 10:00:00
    // снял тестовые сигналы с контрольных пинов,  подправил индикацию обратного отсчета
    // времени съема в режиме внешнего управления
//  v 604g(110) 04.03.2026           (1768212000) 2026.01.12 10:00:00
    // попытка убрать сообщение о форматировании при подключенном кабеле без 
    // разрешения о чтении. 
//  v 604h(111) 06.03.2026           1772780400 //2026/03/06 10:00:00 v111
    // меню отображения в режиме чтения карты SD, индикация подключений
    // новое время релиза
    // при "плохой" дате (когда идет исправление времени) при заствавке 
    // изменяем цвет фона даты и времени
    // новая дата релиза
//  v 604i(112) 10.03.2026           1772780400 //2026/03/06 10:00:00 
    // корректировка индикации подключенной SDCard
//  v 604j(113) 13.03.2026           1772780400 //2026/03/06 10:00:00 
    // улучшен алгоритм поиска событий, приведен к реалиям сбора данных на STM
//  v 604k(114) 16.03.2026           1772780400 //2026/03/06 10:00:00 
    // ориентация курсора на событит по вертикали по графику в середине
    // установка "синего" курсора при просмотре событий в позиции измерения параметров 
    // события - только для отражающих и простых перепадов, для коротких импульсов до 150 нС
//  v 604l(115) 17.03.2026           1772780400 //2026/03/06 10:00:00 
    // изменен порог определения перегрузки сигнала, для поиска конца линии
    // поравлено -  запись о конце линии в файл
//  v 604m(116) 26.03.2026           1772780400 //2026/03/06 10:00:00 
    // введен делитель для расчета смещения при измерениях, 
    // должен быть положительным целым не более 16 , 
    // участвует в корректировке смещения при уровне шумов 
    // в начале больше чем в конце
//  v 604n(117) 31.03.2026           1772780400 //2026/03/06 10:00:00 
    // выключил принудительное трехкратное снижение по 0.7 дБ в шумах
    // на измерениях динамического диапазона
    // выключил на длинных линиях пятиточечное сглаживание 
    // при малоотличимой разнице сигнала
    // включил двуточечный фильтр в малых сигналах, в пределах одного разряда
//  v 604o(118) 02.04.2026           1772780400 //2026/03/06 10:00:00 
    // Цифровые фильтры - обработки сигналов
    // Сейчас:
    // 1) корректор на малых сигналах перед большим импульсом
    // 2) Ф.7.1- малые сигналы - не превышающие смещение в 1 разряд
    // трех точечный фильтр в не лог. данных
    // 3) Ф.6.1 - анализ пяти точек на разницу не более разряда
    // фильтр по 5 точкам
    // 4) Ф.4 - для коротких линий если не было корректировок простой 2 точечный фильтр
    // 5) Ф.8 - Итоговый фильтр двух точечный фильтр для точек с разницей менее разряда,
    // работает всегда по двум точкам, предыдущей преобразованной
    // и текущей также может быть преобразованной. 
    // Команда установки порога поиска смещения в конце линиии по Д.Б.
    // то есть часть от разницы начала и конца для удовлетворительного
    // отображения шумов в конце при расчете динамического диапазона
    // по умолчанию будет по старому, на новой плате она менее "шумная"
    // поставили коэфф shan=5
    // команда работает в установках ;SET:SHAN?<cr> -запрос параметров
    // ;SET:SHAN n<cr> - установка 
    // Вернул табличный перебор сдвигов на короткий линиях.
//  v 604p(119) 02.04.2026           1772780400 //2026/03/06 10:00:00 
    // выключил фильтрацию 2) если правим сигнал перед импульсом
//  v 604q(120) 02.04.2026           1772780400 //2026/03/06 10:00:00
    // коррекция индикации номеров ячеек при работе с памятью измерителя
//  v 604r(121) 22.04.2026           1772780400 //2026/03/06 10:00:00
    // введение дополнительных длительностей импульсов, добавлены 80 и 300 нС.
    // изменения управлением установок импульсов, изменение приемника установок
    // рефлектометра при внешнем управлении, так как индексация импульсов имеет двузначный вид.
    // необходимо проверить в управлении с программой!
//  v 604s(122) 23.04.2026           1772780400 //2026/03/06 10:00:00
    // исправлена таблица установки таймеров для дительности импульса, из-за того что добавили
//  v 604t(123) 21.05.2026           1772780400 //2026/03/06 10:00:00
    // новый комплект длительностей ипульсов, убран 80, 40 заменен 50
    // снова перестройка 
//  v 604u(124) 25.05.2026           1772780400 //2026/03/06 10:00:00
    // пытаемся записать LogFile нажатые кнопки, состояние батарейки
//  v 604v(125) 26.05.2026           1772780400 //2026/03/06 10:00:00
    // исправлен контроль переключения с батарейки на внешнее питание и обратно
//  v 604w(126) 27.05.2026           1772780400 //2026/03/06 10:00:00
    // команды чтения LOG файла, целиком и времена включений ;MMEM:LOG?<cr>  ;MMEM:ONF?<cr>
//  v 604x(127) 28.05.2026           1772780400 //2026/03/06 10:00:00
    // востановление ограничений длительностей импульсов в зависимости
    // от длины линии (диапазона). 
//  v 604y(128) 01.06.2026           1772780400 //2026/03/06 10:00:00
    // ЭКСПЕРИМЕНТАЛЬНАЯ от Д.Б.
    // контроль заряда батарейки по Д.Б., верхний порог на уровне 5.35 (100%)
    // получен экпериментально путем "полной" зарядки (аккумуляторы греются)
    // далее разрядили простым включением на 30 минут и зафиксировали уровень 
    // батареи(которую считываем с делителя BAT_POWER) 
    // нижний порог сообщения оставили 4.15 о низком заряде,
    // индикацию процентов корректируем на 25% быстрее, последние 25%
    // индицируем 0% при этом штатно работаем, так как эти последние % 
    // реально при работе очень быстро садятся 
//  v 604z(129) 02.06.2026           1772780400 //2026/03/06 10:00:00
    // добавил функцию управления яркостью индикатора NEXTION 
    // void SetBrightness(uint8_t Bright)
    // до 100 установка текущей
    // от 101-200 - установка фиксированной после выключения (запоминание)
//  v 605a(130) 03.06.2026           1780308000 //2026/06/01 10:00:00
    // пробуем управлять яркостью дисплея, 40% при ненажимании кнопок в течении 1 минуты
    // откат при нажатии кнопок
//  v 605b(131) 04.06.2026           1780308000 //2026/06/01 10:00:00
    // пересобран проект в CubeMX дял нового пина управлением АЦП 
//  v 605c(132) 30.06.2026           1780308000 //2026/06/01 10:00:00
    // !!! Обнаружена критическая недоработка !!!
    // функционал перепривязки нуля на малых сигналах на этапе измерений 
    // в измерителе НЕ РАБОТАЕТ! Долгое нажатие на кнопку ОК в поле REF,
    // приводит к "повисанию" прибора!
    // Исправлено - долгое нажатие и отпускание кнопки ОК  сразу после писка приводит к перезаписи нуля 
    // на текущее измерение, если продолжать удерживать кнопку то произойдет перезапись в память
    // рассчитаного смещения на самом чувствительном дианпазоне.
    // индикация изменеия и сохранения смещения осуществляется в поле показаний
    // REF. Желтым текущее и измеренное, красным перезаписанное...v605
//  v 605d(133) 21.07.2026           1780308000 //2026/06/01 10:00:00
    // Небольшая правка "супер Теста"
    // сделана 0.5сек задержка при включении что бы ответил индикатор
    // если он совсем НОВЫЙ для перепрограммирования
    // отвечает писком и трех-кратным морганием светодиода "S"
//  v 605e(134) 03.08.2026           1780308000 //2026/06/01 10:00:00
    // убрал "ложное" информирование о режиме измерения перед выдачей признака окончания
    // измерения (END\r) в режиме работы с программой. (ошибочно установлено от "суперТеста")
    //
//  v 605f(135) 13.08.2026           1780308000 //2026/06/01 10:00:00
    // верная таблица установки длительностей импульсов
    // !!!ВНИМАНИЕ!!! эта прошивка обязательна для плат старше партии от 8.26 включительно
//  v 605g(136) 14.08.2026           1780308000 //2026/06/01 10:00:00
...
//  v 605h(137) 18.08.2026           1780308000 //2026/06/01 10:00:00
    // другие точки включения\выключения АЦП
//  v 605i(138) 19.08.2026           1780308000 //2026/06/01 10:00:00
    //RunInRealTime точка контроля - отдельная ветка
//  v 605j(139) 20.08.2026           1780308000 //2026/06/01 10:00:00
    //  выключение питаний рефлектометрической части делается отдельной функцией.
//  v 605k(140) 21.08.2026           1780308000 //2026/06/01 10:00:00
    // вставка из 138 по поводу РеалТайма
//  v 605l(141) 21.08.2026           1780308000 //2026/06/01 10:00:00
    // правильная установка масштаба индикации при измерениях, на полный экран
    // устанавливаем 11 для шкалы более 16...
    // ранее при измерениях устанавливалось не более 1:8 , а при просмотре 
    // после измерений ставилось 11..
//  v 605m(142) 26.08.2026           1780308000 //2026/06/01 10:00:00
    // Перевод асинхроннго ответа "END" в режим синхронного по флажу
    // в ответ на запрос дампа при измерених
//  v 605n(143) 26.08.2026           1780308000 //2026/06/01 10:00:00
    // запись кодов обработаных команд по UART_EXT
//  v 605o(144) 27.08.2026           1780308000 //2026/06/01 10:00:00
    // новый вариант нумерованной записи статусов передачи при приеме команды 
    // запроса дампа (отслеживает сттус передачи)
//  v 605p(145) 28.08.2026           1780308000 //2026/06/01 10:00:00

//  v 605q(146) 03.09.2026           1780308000 //2026/06/01 10:00:00
    // подготовка к перелючению длин волн 1300/1310 для трехволновых рефлектометров
//  v 605r(147) 04.09.2026           1780308000 //2026/06/01 10:00:00
    // исправление критической ошибки при выявлении ошибок в конфигурации
//  v 605s(148) 04.09.2026           1780308000 //2026/06/01 10:00:00
    // реализация подмены 1310/1300 для ФСТЭК  при прописывании признака ФСТЭК
//  v 605t(149) 04.09.2026           1780308000 //2026/06/01 10:00:00
    // добавил команды UART для установки/контроля признака ФСТЭК
    // установка (требует режима установка параметров) - ;set:fsb 1<cr>
    // контроль ;syst:fsb?<cr>
//  v 605u(150) 23.09.2026           1780308000 //2026/06/01 10:00:00
    // Устранен ложный выход по END при управлении от компьютера
//  v 605v(151) 25.09.2026           1780308000 //2026/06/01 10:00:00
    // При сохранениях файлов измерений OLT или OTDR, проход через
    // меню редактирования комментариев и счетчиков волокна а также
    // включение/выключение авто инкремента
    // так же реализована попытка анти дребезга кнопки "S" при вызове
    // сохранения.
